...
@section('heading', 'E-commerce Categories')
@section('subtitle', 'Categories with products in your online store')
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div class="bg-gradient-to-r from-emerald-500 to-emerald-600 rounded-2xl p-6 text-white border-2 border-black">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-emerald-100 text-sm font-medium">Total Categories</p>
    ...
    <div class="bg-white shadow-xl rounded-3xl border border-slate-200 overflow-hidden">
        ...
        <div class="p-6 border-b border-slate-200">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                    <h2 class="text-2xl font-bold text-slate-900">📂 Category Catalog</h2>
                    <p class="text-sm text-slate-600 mt-1">Categories are auto-populated from products added to e-commerce</p>
                </div>
            </div>
        </div>
        ...
        <div class="p-4 bg-slate-50 border-b border-slate-200">
            <form method="GET" action="{{ route('inventory.ecommerce-categories.index') }}" class="flex flex-wrap gap-3 items-end">
                <div class="flex-1 min-w-[200px]">
                    <label class="text-xs font-medium text-slate-500 mb-1 block">Search Categories</label>
        ...
        <div class="p-6">
            @if($categories->count() > 0)
                <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
                    @foreach($categories as $category)
                        <a href="{{ route('inventory.ecommerce-categories.show', $category) }}" 
                           class="group bg-white border border-slate-200 rounded-xl p-4 hover:shadow-lg hover:border-emerald-300 transition-all duration-200">
                            <div class="aspect-square rounded-lg bg-gradient-to-br from-emerald-100 to-emerald-50 flex items-center justify-center mb-3 overflow-hidden">
                                @if($category->image)
                            ...
                            <h3 class="text-sm font-semibold text-slate-900 text-center truncate group-hover:text-emerald-600 transition">
                                {{ $category->name }}
                            </h3>
                            <p class="text-xs text-slate-500 text-center mt-1">
                                {{ $category->ecommerce_products_count }} {{ Str::plural('product', $category->ecommerce_products_count) }}
                            </p>
                        </a>
                    @endforeach
                </div>
            ...
            @else
                <div class="text-center py-12">
                    ...
                    <h3 class="text-lg font-semibold text-slate-900 mb-1">No categories found</h3>
                    <p class="text-sm text-slate-500">Categories will appear here when you add products to e-commerce.</p>
                    <a href="{{ route('inventory.ecommerce-products.create') }}" 
                       class="inline-flex items-center mt-4 px-4 py-2 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 text-sm font-medium transition">
