                    ...
                    <div class="space-y-3">
                        ...
                        <div class="border border-slate-200 rounded-lg overflow-hidden">
                            <label class="flex items-start cursor-pointer p-4 bg-slate-50 hover:bg-slate-100 transition-colors">
                                {{-- ✅ NO onchange here — JS handles it --}}
                        ...
                        <div class="border border-slate-200 rounded-lg overflow-hidden">
                            <label class="flex items-start cursor-pointer p-4 bg-slate-50 hover:bg-slate-100 transition-colors">
                                ...
                                <div class="flex-1">
                                    ...
                                    <div class="text-xs text-slate-500">Process payment through Khalti integration</div>
                                </div>
                            </label>
                            ...
                            <div id="integrated-fields" class="p-4 bg-white border-t border-slate-200 hidden">
                                <div class="bg-emerald-50 border border-emerald-200 p-3 rounded-lg mb-3">
                                    <div class="text-sm text-emerald-700">
                                        <strong>Integrated Payment:</strong> Click "Pay with Khalti" below to open the
                                        payment popup. Payment will be recorded automatically after success.
                                    </div>
                                </div>

                                {{-- Hidden field so payment_method is always sent --}}
                                <input type="hidden" name="payment_method" id="integrated-payment-method" value="khalti">

                                <div class="flex items-center space-x-3 p-3 bg-purple-50 border border-purple-200 rounded-lg mb-3">
                                    <div class="text-purple-700 font-medium">💜 Khalti Payment Gateway</div>
                                    <div class="text-xs text-purple-600">Secure online payment</div>
                                </div>

                                <!-- Pay with Khalti Button -->
                                <!-- Pay with Khalti Button — using local Khalti logo asset -->
<button type="button" id="khalti-pay"
    ...
    style="background-color: #ffffff; border: 2px solid #e53e3e; color: #e53e3e;"
    onmouseover="this.style.boxShadow='0 4px 12px rgba(229, 62, 62, 0.3)'"
    onmouseout="this.style.boxShadow='none'"
    onmousedown="this.style.backgroundColor='#e53e3e'; this.style.color='#ffffff';"
    onmouseup="this.style.backgroundColor='#ffffff'; this.style.color='#e53e3e';">
...
<button type="button" id="esewa-pay"
    class="mt-2 inline-flex items-center gap-2 px-5 py-2.5 rounded-lg transition duration-200 hover:shadow-lg active:scale-95"
    style="background-color: #0af802ff; border: 2px solid #ffffffff; color: #ffffffff;"
    onmouseover="this.style.boxShadow='0 4px 12px rgba(0, 253, 17, 0.3)'"
    onmouseout="this.style.boxShadow='none'"
    onmousedown="this.style.backgroundColor='#e53e3e'; this.style.color='#ffffff';"
    onmouseup="this.style.backgroundColor='#ffffff'; this.style.color='#e53e3e';">
    ...
    <span class="font-semibold text-sm">Pay with esewa</span>
                                ...
                                <!-- Test SDK Button (remove in production) -->
                                <!-- <button type="button" id="test-khalti"
                                        class="mt-2 ml-2 px-4 py-2.5 bg-slate-500 text-white rounded-lg hover:bg-slate-600 transition duration-200 text-sm">
                                    🔍 Test SDK
                                </button> -->
                            </div>
                        </div>
            ...
            <div class="flex justify-between items-center pt-6 border-t">
                ...
                <div class="flex gap-3">
                    ...
                    <!-- Integrated payment — no submit button, only Khalti button above -->
                    <div id="integrated-submit-buttons" class="hidden">
                        ...
                        <span class="px-6 py-2.5 bg-slate-200 text-slate-400 rounded-xl text-sm cursor-not-allowed">
                            Use "Pay with Khalti" button above ↑
                        </span>
                    </div>
                </div>
            </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        // ── Khalti Pay Button — redirects to Khalti website ──
document.getElementById('khalti-pay')?.addEventListener('click', function (e) {
    ...
    if (!amount || amount <= 0) { alert('⚠️ Please enter a valid amount.'); return; }

    const form = document.getElementById('payment-form');

    // ✅ Use setAttribute instead of property assignment
    form.setAttribute('action', "{{ route('pos.khalti.initiate') }}");
    form.setAttribute('method', 'POST');

    // ✅ Ensure CSRF token exists
    let csrf = form.querySelector('input[name="_token"]');
    if (!csrf) {
...
document.getElementById('esewa-pay')?.addEventListener('click', function (e) {
    ...
    if (!token) {
        // fallback: try to get token from existing hidden input in the main form
    ...
    esewaForm.action = "{{ route('pos.esewa.initiate') }}";
    ...
    esewaForm.appendChild(csrfInput);
    ...
    const fields = {
        ...
        business_account: document.querySelector('select[name="business_account"]').value,
        note:             document.querySelector('textarea[name="note"]').value,
        bank_charge:      document.querySelector('input[name="bank_charge"]').value || 0,
    ...
    esewaForm.submit();
        ...
        // ── Test Button ──
        document.getElementById('test-khalti')?.addEventListener('click', function () {
            alert('✅ Khalti redirect mode active!\nClick "Pay with Khalti" to proceed.');
        });
