            ...
            <table class="w-full text-sm" id="itemsTable">
                <thead class="text-slate-700 bg-slate-100">
                    <tr>
                        ...
                        <th class="text-left px-4 py-3 font-medium">Taxes</th>
                        <th class="text-left px-4 py-3 font-medium">Tax Amount</th>
                ...
                <tfoot class="bg-slate-50 border-t-2 border-slate-300">
                    ...
                    <tr>
                        <td colspan="5" class="px-4 py-3 text-right font-semibold text-slate-700">Total Tax:</td>
                        <td class="px-4 py-3 font-semibold text-red-600 text-right" id="totalTax">0.00</td>
                        <td colspan="4"></td>
                    </tr>
...
function calculateRowTotal(rowId) {
    ...
    // Taxes
    let taxAmount = 0;
    const selectedTaxesInput = row.querySelector('.selected-taxes-input');
    const selectedTaxes = selectedTaxesInput ? JSON.parse(selectedTaxesInput.value || '[]') : [];

    selectedTaxes.forEach(taxId => {
        const tId = Number(taxId);
        const tax = taxes.find(t => t.id === tId);
        if (!tax) return;

        if (tax.type === 'fixed') {
            taxAmount += (Number(tax.rate) || 0) * qty;
        } else {
            taxAmount += (baseCost * (Number(tax.rate) || 0)) / 100;
        }
    });
    ...
    return { baseCost, taxAmount, subtotal };
}
...
// ---------- Taxes popup (keep yours but FIX ids) ----------
function showTaxPopup(rowId) {
    if (activeTaxPopup) {
        activeTaxPopup.remove();
        activeTaxPopup = null;
    }
    removeAutocomplete();
    ...
    const selectedTaxesInput = row.querySelector('.selected-taxes-input');
    const selectedTaxes = selectedTaxesInput ? JSON.parse(selectedTaxesInput.value || '[]').map(Number) : [];

    const popup = document.createElement('div');
    popup.id = `tax-popup-${rowId}`;
    popup.className = 'absolute bg-white border border-slate-200 rounded-lg shadow-xl';
    ...
    let taxCheckboxes = '';
    taxes.forEach(tax => {
        const isChecked = selectedTaxes.includes(Number(tax.id));
        taxCheckboxes += `
        <label class="flex items-center gap-2 text-xs p-3 hover:bg-slate-50 cursor-pointer border-b border-slate-100 last:border-b-0">
            <input type="checkbox" value="${tax.id}" ${isChecked ? 'checked' : ''}>
            <span class="text-slate-700 flex-1">${tax.name}</span>
            <span class="text-slate-500">${tax.formatted}</span>
        </label>`;
    });

    popup.innerHTML = `
        <div class="p-2 max-h-60 overflow-y-auto">
            <div class="text-xs font-semibold text-slate-600 px-3 py-2">Select Taxes:</div>
            ${taxCheckboxes}
        </div>
        <div class="border-t border-slate-200 p-3">
            <button type="button"
                    class="w-full px-4 py-2 bg-blue-600 text-white text-sm rounded hover:bg-blue-700 font-medium">
                Apply Taxes
            </button>
        </div>
    `;

    popup.querySelector('button').addEventListener('click', () => saveTaxSelection(rowId));

    const taxButton = row.querySelector('.tax-button');
    const rect = taxButton.getBoundingClientRect();
    ...
    activeTaxPopup = popup;

    setTimeout(() => {
        const close = (e) => {
            if (activeTaxPopup && !activeTaxPopup.contains(e.target) && e.target !== taxButton && !taxButton.contains(e.target)) {
                saveTaxSelection(rowId);
                document.removeEventListener('click', close);
            }
        };
        document.addEventListener('click', close);
...
function saveTaxSelection(rowId) {
    const row = document.getElementById(`row-${rowId}`);
    if (!row) return;

    const popup = document.getElementById(`tax-popup-${rowId}`);
    if (!popup) return;

    const checked = popup.querySelectorAll('input[type="checkbox"]:checked');
    const selectedTaxIds = Array.from(checked).map(cb => Number(cb.value));

    // store ids as numbers
    const selectedTaxesInput = row.querySelector('.selected-taxes-input');
    selectedTaxesInput.value = JSON.stringify(selectedTaxIds);

    // label
    const displaySpan = row.querySelector('.selected-taxes-display');
    if (selectedTaxIds.length === 0) {
        displaySpan.textContent = 'Select Taxes';
        displaySpan.className = 'selected-taxes-display text-slate-500 text-sm';
    } else if (selectedTaxIds.length === 1) {
        const tax = taxes.find(t => t.id === selectedTaxIds[0]);
        displaySpan.textContent = tax ? tax.name : '1 tax';
        displaySpan.className = 'selected-taxes-display text-blue-600 text-sm font-medium';
    } else {
        displaySpan.textContent = `${selectedTaxIds.length} taxes`;
        displaySpan.className = 'selected-taxes-display text-blue-600 text-sm font-medium';
    }

    // hidden inputs for POST
    const taxInputsContainer = row.querySelector('.tax-inputs-container');
    taxInputsContainer.innerHTML = '';
    selectedTaxIds.forEach(taxId => {
        const input = document.createElement('input');
        input.type = 'hidden';
        input.name = `items[${rowId}][taxes][]`;
        input.value = taxId;
        taxInputsContainer.appendChild(input);
    });
    ...
    row.innerHTML = `
       ...
       class="cost-input w-full rounded-lg border-slate-300 focus:border-blue-500 focus:ring-blue-500 text-sm px-3 py-1.5 text-right">
        ...
        <td class="px-4 py-3">
            <div class="relative">
                <button type="button"
                        class="tax-button w-full px-3 py-2 rounded border border-slate-300 bg-white hover:bg-slate-50 text-sm text-left flex justify-between items-center">
                    <span class="selected-taxes-display text-slate-500 text-sm">Select Taxes</span>
                    <svg class="w-3 h-3 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
                    </svg>
                </button>
                <div class="tax-inputs-container"></div>
                <input type="hidden" class="selected-taxes-input" value="[]">
            </div>
        </td>
        <td class="px-4 py-3">
            <div class="text-red-600 font-medium text-sm">
                <span class="tax-amount">0.00</span>
                <input type="hidden" name="items[${rowId}][tax_amount]" class="tax-amount-input" value="0">
            </div>
        </td>
        <td class="px-4 py-3">
...
// show tax popup
row.querySelector('.tax-button').addEventListener('click', () => showTaxPopup(rowId));
...
function removeRow(rowId) {
    ...
    if (row) row.remove();
    removeAutocomplete();
    if (activeTaxPopup) { activeTaxPopup.remove(); activeTaxPopup = null; }
    updateAllTotals();
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.addEventListener('keydown', function(e) {
        if (e.key === 'Escape') {
            removeAutocomplete();
            if (activeTaxPopup) {
                const popupId = activeTaxPopup.id;
                const rowId = popupId.replace('tax-popup-', '');
                saveTaxSelection(rowId);
            }
        }
    });
