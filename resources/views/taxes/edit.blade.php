...
@section('subtitle', 'Update tax configuration')
@section('heading', 'Edit Tax')
...
<div class="max-w-2xl mx-auto">
  ...
  <div class="bg-white rounded-xl border border-slate-200 p-6">
    ...
    <p class="text-sm text-slate-500 mb-6">Update the tax configuration details.</p>
    ...
    <form action="{{ route('taxes.update', $tax) }}" method="POST" class="space-y-6">
      ...
      <div>
        ...
        <div class="space-y-3">
          <label class="flex items-center">
            <input type="radio" name="type" value="percentage" {{ old('type', $tax->type) === 'percentage' ? 'checked' : '' }}
                   class="w-4 h-4 text-green-600 border-slate-300 focus:ring-green-500">
            <span class="ml-2 text-sm text-slate-700">Percentage (%)</span>
          </label>
          <label class="flex items-center">
            <input type="radio" name="type" value="fixed" {{ old('type', $tax->type) === 'fixed' ? 'checked' : '' }}
                   class="w-4 h-4 text-green-600 border-slate-300 focus:ring-green-500">
      ...
      <div>
        ...
        <div class="relative">
          <input type="number" id="rate" name="rate" value="{{ old('rate', $tax->rate) }}" required
